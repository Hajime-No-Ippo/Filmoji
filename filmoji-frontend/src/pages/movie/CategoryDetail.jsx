...
function CategoryDetail() {
  ...
  useEffect(() => {
    fetch('/api/movies')
      ...
      .then((data) => {
        const filtered = data
          .filter((m) => m.genres?.toLowerCase().includes(decodedName.toLowerCase()))
    ...
    return (
      <div className="min-h-screen flex items-center justify-center text-white pt-20">
        <div className="text-center">
          ...
          <Link to="/categories" className="accent-link">← Back to Categories</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-dark min-h-screen pt-20 pb-20">
      <div className="container-main px-4">
        <Link to="/categories" className="nav-link flex items-center gap-2 text-sm mb-8 inline-block">
          ← Back to Categories
        </Link>
        ...
        <p className="section-subtitle mb-4">Top 5 Highest Rated {decodedName} Movies</p>
        ...
        {loading ? (
              ...
              >
                ...
                <div className="flex-shrink-0 text-right">
                  <div className="flex items-center gap-2 justify-end mb-2">
                    ...
                    <span className="text-ink font-bold text-2xl">{movie.rating?.toFixed(1)}</span>
                  </div>
          ...
          <div className="text-center py-12">
            <p className="text-muted text-lg">No movies found in the {decodedName} category.</p>
          </div>
        )}
      </div>
    </div>
